    ...
    insertUserData(userData){
        ...
        db.transaction((tx)=>{
            for(let i=0; i < len; i++){
                ...
                let sendState = user.sendState;
                let sql = "INSERT INTO user(selfUuid,hxId,otherUuid,isOther,msgType,delay,url,data,sendState,isReaded)"+
                "values(?,?,?,?,?,?,?,?,?,?)";
                tx.executeSql(sql,[selfUuid,hxId,otherUuid,isOther,msgType,delay,url,data,sendState,isReaded],()=>{
                },(err)=>{
                    console.log(err);
                }
            );
